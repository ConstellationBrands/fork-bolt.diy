 ...
 * WebView. It exposes four primitives:
 *
 *   - createSecurityHeaders(env, request) -> Record<string, string>
 *   - checkRateLimit(request, endpoint, store?) -> Promise<{ allowed, ... }>
 *   - enforceCsrf(request, env) -> Response | null
 *   - withSecurity(handler, opts) -> wrapped handler
 */
...
/** KV-backed store; `kv` is a Cloudflare KVNamespace. Typed loosely so we
 *  don't pull in @cloudflare/workers-types as a runtime dep. */
...
class KvRateLimitStore implements RateLimitStore {
  ...
  async delete(key: string) {
    try {
      ...
      // Never block user traffic on a KV delete error.
    }
  }
}
 ...
 *  distributed store. Safe to call with `undefined` — keeps the in-memory fallback. */
...
export function createSecurityHeaders(env?: EnvLike, request?: Request): Record<string, string> {
   ...
   *    Vite/HMR work; in production we drop it.
   *  - The WebContainer worker iframe runs in its own origin and is covered
   *    by `frame-src blob:`.
   *  - `connect-src` is permissive because the agent fans out to many
   *    provider APIs. We keep it `https:` + `wss:` rather than blanket-allow
   *    http, which blocks mixed-content downgrade attacks.
   */
...
export function enforceCsrf(request: Request, env?: EnvLike): Response | null {
  ...
  const sameOrigin =
    (origin && tryHost(origin) === expectedHost) || (referer && tryHost(referer) === expectedHost);
  ...
  // cookies yet. Keep the list narrow.
...
export function withSecurity<T extends (args: ActionFunctionArgs | LoaderFunctionArgs) => Promise<Response>>(
  ...
  return async (args: ActionFunctionArgs | LoaderFunctionArgs): Promise<Response> => {
    ...
    // CSRF is opt-in (requireCsrf: true) to avoid breaking existing routes
    // that haven't been updated to use securedFetch.
