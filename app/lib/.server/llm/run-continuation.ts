...
const SCAFFOLD_RE =
  /create-vite|npm\s+create\s+vite|pnpm\s+dlx\s+create-vite|create-react-app|scaffold/i;
...
export function analyzeRunContinuation(options: RunContinuationOptions): RunContinuationDecision {
  ...
  if (alreadyAttempted) {
    return { shouldContinue: false, reason: 'already-attempted' };
  }
  ...
  if (!runIntentDetected && !buildIntentDetected && !starterBootstrapDetected) {
    return { shouldContinue: false, reason: 'no-run-or-build-intent' };
  }
  ...
  if (!hasAnyBoltAction) {
    return { shouldContinue: true, reason: 'no-bolt-actions' };
  }

  if (onlyInspectionCommands) {
    return { shouldContinue: true, reason: 'inspection-only-shell-actions' };
  }

  if ((mentionsScaffold || starterBootstrapDetected) && !hasStartAction && !hasImplementationFile) {
    return { shouldContinue: true, reason: 'scaffold-without-start' };
  }

  if (runIntentDetected && !hasStartAction) {
    return { shouldContinue: true, reason: 'run-intent-without-start' };
  }
  ...
  if (
    ...
    return { shouldContinue: true, reason: 'starter-without-implementation' };
  }

  if (buildIntentDetected && onlyBootstrapCommands && !hasImplementationFile) {
    return { shouldContinue: true, reason: 'bootstrap-only-shell-actions' };
  }
