...
export function PushToGitHubDialog({ isOpen, onClose, onPush }: PushToGitHubDialogProps) {
  ...
  useEffect(() => {
    console.log('bef open conn', getLocalStorage('github_connection'))
    if (isOpen) {
      console.log('open conn', getLocalStorage('github_connection'))
      const connection = getLocalStorage('github_connection');

      if (connection?.user && connection?.token) {
        console.log('bef conn', connection?.user)
        setUser(connection.user);
        console.log('aft conn', connection?.user)
      }
    }
  }, [isOpen]);
