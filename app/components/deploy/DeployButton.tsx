...
import { useDDPDeploy } from '~/components/deploy/DDPDeploy.client';
...
export const DeployButton = ({ onVercelDeploy, onNetlifyDeploy }: DeployButtonProps) => {
  ...
  const [deployingTo, setDeployingTo] = useState<'netlify' | 'vercel' | 'ddp' | null>(null);
  const isStreaming = useStore(streamingState);
  const { handleVercelDeploy } = useVercelDeploy();
  const { handleNetlifyDeploy } = useNetlifyDeploy();
  const { handleDDPDeploy, DDPDeployDialog: ddpDeployDialog } = useDDPDeploy();
  ...
  const handleDeployToDDPClick = async () => {
    setIsDeploying(true);
    setDeployingTo('ddp');

    try {
      await handleDDPDeploy();
    } finally {
      setIsDeploying(false);
      setDeployingTo(null);
    }
      ...
      <div className="flex border border-bolt-elements-borderColor rounded-md overflow-hidden text-sm">
        ...
        >
          ...
          <DropdownMenu.Item
            ...
            onClick={handleDeployToDDPClick}
          >
            <div className="w-5 h-5 flex items-center justify-center">
              <span className="i-ph:github-logo-fill text-lg" />
            </div>
            <span className="mx-auto">Deploy to DDP (GitHub)</span>
          </DropdownMenu.Item>
