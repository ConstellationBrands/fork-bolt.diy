 ...
 *   2. Fixed-window rate limit per client IP on /api/* routes.
 *   3. Merge a standard security-header bag into every response (CSP,
 *      HSTS, X-Frame-Options, COOP/COEP, etc.).
 *   4. Route-level exceptions for the WebContainer connect/preview pages,
...
export const onRequest: PagesFunction<PagesEnv> = async (context) => {
  ...
  // 2. Rate-limit on API routes only.
  ...
  for (const [key, value] of Object.entries(security)) {
    // 4. The WebContainer preview/connect routes need relaxed COEP so the
